...
if ($_GET["Command"] == "save_item") {
    ...
    try {
        ...
        $sql = "Insert into m_item(REF, category_name, school_ref, school_name, item_name, des, user, listtype,img)values
                        ('" . $no1 . "' ,'" . $_GET['category_name'] . "' ,'" . $_GET['school_ref'] . "' ,'" . $_GET['school_name'] . "'  ,'" . $_GET['item_name'] . "' ,'" . $_GET['des'] . "', '" . $_SESSION['UserName'] . "','CAR','" . $_GET['img_logo'] . "')";
        $result = $conn->query($sql);
...
if ($_GET["Command"] == "cancel_imb") {
    try {
        ...
        $result = $conn->query($sql);
        $conn->commit();
        echo "Cancel Card n Flowers successfully";
