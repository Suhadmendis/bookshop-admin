...
if ($_GET["Command"] == "generate") {
    ...
    $sql      = "SELECT level_type_ref FROM sys_info";
    $result   = $conn->query($sql);
    $row      = $result->fetch();
    $no       = $row['level_type_ref'];
    $tmpinvno = "000000" . $row["level_type_ref"];
    $lenth    = strlen($tmpinvno);
    $no       = trim("LVE/") . substr($tmpinvno, $lenth - 7);
...
if ($_GET["Command"] == "save_item") {
    ...
    try {
        ...
        $tmpinvno = "000000" . $row["level_type_ref"];
        $lenth    = strlen($tmpinvno);
        $no1      = trim("LEV/") . substr($tmpinvno, $lenth - 7);

        $REF_GET = $_GET['REF'];
        $name =$_GET['name'];
        $sql    = "SELECT  `REF` FROM `m_level_type`  WHERE REF = '" . $REF_GET . "'";
        $result = $conn->query($sql);
        $row    = $result->fetchall();
        ...
        if (isset($REF_GET) && count($row) >= 1) {
            ...
            $sql    = "UPDATE `m_level_type` SET `name`='" . $name . "' WHERE REF = '" . $REF_GET . "'";
            $result = $conn->query($sql);
            $conn->commit();
            echo 'Updated Level Type successfully';

        } else {

            $sql    = "Insert into m_level_type(REF, name, user)values
    ('" . $no1 . "' ,'" . $_GET['name'] . "','" . $_SESSION['UserName'] . "')";
            $result = $conn->query($sql);


            $no2    = $no + 1;
            $sql    = "update sys_info set level_type_ref = $no2 where level_type_ref = $no";
            $result = $conn->query($sql);
...
if ($_GET["Command"] == "getForm") {
    ...
    $sql = "select * from m_level_type where REF= '" . $REF . "'";
