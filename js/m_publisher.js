var vue = new Vue({
  ...
  mounted() {
    axios.get("m_publisher_data.php?Command=generate").then((response) => {
...
function getFromValues() {
  ...
  if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
    ...
    if (IDF === "item") {
      opener.document.getElementById("publisher_ref").value = objSup.REF;
