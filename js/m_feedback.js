var vue = new Vue({
    ...
    mounted() {
        axios.get("m_feedback_data.php?Command=generate").then((response) => {
...
function getFromValues() {
    ...
    if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
        ...
        if (IDF === "Master") {
            ...
            opener.document.getElementById("feedback").value = objSup.feedback;
        }
        if (IDF === "item") {
            opener.document.getElementById("feedback_ref").value = objSup.REF;
