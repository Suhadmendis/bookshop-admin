var vue = new Vue({
  ...
  methods: {
    onStartUp: function () {
      axios
        .get("m_student_verification_data.php?Command=generate")
        .then((response) => {
          this.en_name = response.data[1];
          this.REF = response.data[0];
          this.STUDENTS = response.data[2];
        });
    },
    approve: function (fname, lname, agepoint) {
    //   alert(agepoint);
        axios.get("m_student_verification_data.php?Command=approve&fname=" + fname + "&lname=" + lname + "&agepoint=" + agepoint).then((response) => {
            alert("Approved");
            this.onStartUp();
        });
    },
...
//     var url = "m_student_verification_data.php";
